...
# 使用 torch.compiler 加速的 mxfp8_torch 函数
@torch.compile
def mxfp8_torch_compiled(x:torch.Tensor, 
                         stochastic_rounding:bool=False, 
                         scaled_value_format:str='e4m3') -> torch.Tensor:
    """
    使用 torch.compiler 加速的 MXFP8 量化函数
    """
    if scaled_value_format == "e4m3":
        fp8_max = 448
    elif scaled_value_format == "e5m2":
...
def mxfp8_torch_compiled_optimized(x:torch.Tensor, 
                                  stochastic_rounding:bool=False, 
                                  scaled_value_format:str='e4m3') -> torch.Tensor:
    """
    使用 torch.compiler 优化的 MXFP8 量化函数，减少类型转换开销
    """
    if scaled_value_format == "e4m3":
        fp8_max = 448
    elif scaled_value_format == "e5m2":
    ...
    scale = torch.pow(2.0, torch.floor(torch.log2(fp8_max / max_vals)))
    scale = torch.clamp(scale, min=1.0, max=torch.inf)
    
    # 减少类型转换
    x_scaled = x_abs * scale
    
    if scaled_value_format == "e4m3":
        x_fp8_abs = x_scaled.to(torch.float8_e4m3fn).to(x.dtype) / scale
    elif scaled_value_format == "e5m2":
        x_fp8_abs = x_scaled.to(torch.float8_e5m2).to(x.dtype) / scale
    
    return sign * x_fp8_abs


# 批量处理优化版本
@torch.compile
def mxfp8_torch_batch_optimized(x:torch.Tensor, 
                                stochastic_rounding:bool=False, 
                                scaled_value_format:str='e4m3',
                                batch_size:int=1024) -> torch.Tensor:
    """
    批量处理的 MXFP8 量化函数，适用于大张量
    """
    if scaled_value_format == "e4m3":
        fp8_max = 448
    elif scaled_value_format == "e5m2":
    ...
    for i in range(0, x_flat.shape[0], batch_size):
        ...
        # 计算批次内的缩放
        batch_max = batch_abs.max()
        scale = torch.pow(2.0, torch.floor(torch.log2(fp8_max / batch_max)))
        scale = torch.clamp(scale, min=1.0, max=torch.inf)
        
        batch_scaled = batch_abs * scale
        
        if scaled_value_format == "e4m3":
            batch_fp8 = batch_scaled.to(torch.float8_e4m3fn).to(x.dtype) / scale
        elif scaled_value_format == "e5m2":
            batch_fp8 = batch_scaled.to(torch.float8_e5m2).to(x.dtype) / scale
        
        results.append(batch_sign * batch_fp8)
